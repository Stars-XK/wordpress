<section class="hero-section">
    <div class="hero-content">
        <div class="container">
            <div class="hero-text">
                <h1 class="hero-title">
                    <?php echo get_bloginfo('name'); ?>
                </h1>
                <p class="hero-description">
                    <?php echo get_bloginfo('description'); ?>
                </p>
                <div class="hero-buttons">
                    <a href="<?php echo esc_url(get_post_type_archive_link('post')); ?>" class="btn btn-primary">
                        <?php esc_html_e('Explore', 'godmaincode'); ?>
                    </a>
                    <a href="<?php echo esc_url(get_page_link(get_page_by_path('about'))); ?>" class="btn btn-secondary">
                        <?php esc_html_e('About Us', 'godmaincode'); ?>
                    </a>
                </div>
            </div>
            
            <div class="hero-stats">
                <div class="stat-card">
                    <span class="stat-number"><?php echo wp_count_posts('post')->publish; ?></span>
                    <span class="stat-label"><?php esc_html_e('Articles', 'godmaincode'); ?></span>
                </div>
                <div class="stat-card">
                    <span class="stat-number"><?php echo wp_count_comments()->approved; ?></span>
                    <span class="stat-label"><?php esc_html_e('Comments', 'godmaincode'); ?></span>
                </div>
                <div class="stat-card">
                    <span class="stat-number"><?php echo count_users()['total_users']; ?></span>
                    <span class="stat-label"><?php esc_html_e('Users', 'godmaincode'); ?></span>
                </div>
            </div>
        </div>
    </div>
    <div class="hero-wave">
        <svg viewBox="0 0 1440 120" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M0 120L60 110C120 100 240 80 360 70C480 60 600 60 720 65C840 70 960 80 1080 85C1200 90 1320 90 1380 90L1440 90V120H1380C1320 120 1200 120 1080 120C960 120 840 120 720 120C600 120 480 120 360 120C240 120 120 120 60 120H0Z" fill="white"/>
        </svg>
    </div>
</section>